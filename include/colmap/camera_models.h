// Camera models used by COLMAP. Ceres-solver dependency removed (cannot be used in optimization!)
...
void BaseCameraModel<CameraModel>::IterativeUndistortion(const T* params, T* u,
  ...
  for (size_t i = 0; i < kNumIterations; ++i) {
    ...
    x -= step_x;
    if (step_x.squaredNorm() < kMinStepNorm) {
      break;
    }
  }
...
template <typename T>
void SimplePinholeCameraModel::Distortion(const T* extra_params, const T u,
                                          const T v, T* du, T* dv) {
  (void) extra_params;
...
template <typename T>
void PinholeCameraModel::Distortion(const T* extra_params, const T u,
                                    const T v, T* du, T* dv) {
  (void) extra_params;
